...
public:
	...
	bool is_leapyear();
	long milli_seconds_difference(datetime anotherDT) const;
...
protected:
	...
	struct timeval timet_to_timeval(time_t argument);

	double millitickseconds = 0;
	struct timeval timetTotimeval(time_t argument);

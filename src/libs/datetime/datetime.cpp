...
namespace jed_utils {
uint64_t datetime::timeSinceEpochMillisec() {
...
datetime::datetime() :
	...
	time_t timlocal = mktime(tm_now);
	timeval tval = timet_to_timeval(timlocal);
	//millitickseconds = tval.tv_sec * 1000 + (tval.tv_usec / 1000.0);
	millitickseconds = timeSinceEpochMillisec() / 1000.0;
}
...
datetime::datetime(const datetime &other) {
	...
	millitickseconds = other.millitickseconds;
}
...
datetime &datetime::operator=(const datetime &other) {
	if (this != &other) {
		_copy_from(other.timeInfo);
		this->millitickseconds = other.millitickseconds;
	}
	return *this;
}
...
datetime::datetime(datetime &&other) noexcept
	...
	millitickseconds = other.millitickseconds;
	//other.millitickseconds = millitickseconds;
}
...
struct timeval datetime::timet_to_timeval(time_t argument) {
	/*----------------------------------------------------------------------------
...
double datetime::get_ticks() const {
	return millitickseconds;
}

long datetime::get_milliseconds() {
	return (((long)millitickseconds) % 1000);
}
...
void datetime::set_ticks(double ticks) {
	millitickseconds = ticks;
}
...
long datetime::milli_seconds_difference(datetime anotherDT) const {
	//long ticksDiff = clockTime - anotherDT.clockTime;
	double dbdtime = millitickseconds - anotherDT.get_ticks();
	return long(dbdtime * 1000.0);
}

timespan operator-(const datetime &mdt, const datetime &odt) {
	int days = 0, hours = 0, minutes = 0, seconds = 0;

	long milli_diff = mdt.milli_seconds_difference(odt);
	...
	timespan retVal(days, hours, minutes, seconds, milli_diff);
	return retVal;
}
} // namespace jed_utils